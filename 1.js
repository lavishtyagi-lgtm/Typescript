...
rl.question("Enter a number:", function (answer) {
    ...
    var ans = palindrome(value);
    for (var i = 0; i < ans.length; i++) {
        if (ans[i] === ans[ans.length - i - 1]) {
            continue;
        }
        else {
            console.log("It is not a palindrom number ".concat(val));
            rl.close();
            return;
        }
    }
    console.log("It is a Palindrom number ".concat(val));
    rl.close();
});
